...
float divideSDF(float distA, float distB) {
    return (distA/distB);
}

float divideOtherSDF(float distA, float distB) {
    return (distB/distA);
}

float multiple(float distA, float distB) {
    return (distA *distB);
}
...
float scene(vec3 position){
    ...
    vec3 spherepos =  vec3(
            ...
            position.y + sin(time*.5)/10., 
            position.z + (sin(time/10.0)+.4));
        
    pMod3(spherepos,vec3(2.5));
    ...
    // position = vec3(noise(position))*10;
    ...
    float noiz = vfbm(vec2(position.x, position.y), 2)/300.0; 
     noiz = rmf(vec2(position.x, position.y), 1)/3.0; 
    float ground = position.y+.9 + (sin(position.x * 2.+ position.z)) / 2. 
                              + cos(position.z * 3.) / 3. + 5.;
    ...
    // return min(addTwo,sphere);
    return (addTwo*sphere);
}
...
vec4 scenecolor(vec3 position){
    ...
    vec3 sphere3d = hsv2rgb(vec3(0.85+(sphere*0.2),0.4,0.6));
    // vec3 sphere3d = (vec3(sphere,sphere,sphere));
    // vec3 ground3d = hsv2rgb(vec3(1.0,.5,.9));
    vec3 ground3d = vec3(ground,.5,.5);
...
// 	dir = lookAt(pos, camOrigin, rayOrigin);
    ...
    // color = color* scenecolor(vec3(pos.x,pos.y, 0.0));
    ...
    vec3 hsv = hsv2rgb(vec3(.8*(color.x+(time/10.0))*.9, color.x,color.x));
    color = vec4(hsv.x,hsv.y,hsv.z,color.x);
